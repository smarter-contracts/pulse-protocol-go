...
type ConsentStore interface {
	...
	StoreConsent(ctx context.Context, cid string, record *PulseECEncryptionResult) error

	// GetConsent retrieves an encrypted consent record by CID.
	// Returns ErrConsentNotFound if no record exists for the given CID.
	GetConsent(ctx context.Context, cid string) (*PulseECEncryptionResult, error)
	...
	StoreRevocation(ctx context.Context, grantCID, revokeCID string, record *PulseRevokePayload) error

	// GetRevocation retrieves an encrypted revocation record by its CID.
	// Returns ErrConsentNotFound if no record exists for the given CID.
	GetRevocation(ctx context.Context, revokeCID string) (*PulseRevokePayload, error)
}
...
type ConsentStoreMulti interface {
	StoreConsent(ctx context.Context, cid string, record *PulsePQEncryptionResult) error
	GetConsent(ctx context.Context, cid string) (*PulsePQEncryptionResult, error)
	ListConsents(ctx context.Context, filter ConsentFilter) ([]ConsentSummary, error)
	StoreRevocation(ctx context.Context, grantCID, revokeCID string, record *PulseRevokePayload) error
	GetRevocation(ctx context.Context, revokeCID string) (*PulseRevokePayload, error)
}
