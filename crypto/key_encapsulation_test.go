 ...
 * Alice Private Key: <contents of alice_private.hex>
 * Bob Public Key: <contents of bob_public.hex>
 *
 ...
 *      - First 1152 bytes -> 'Actual' Private Key
 *      - Next 1184 bytes -> Public Key
 *      - Next 32 bytes -> Hash of Public Key
 *      - Last 32 bytes -> "Z"
*
 *    ChainId = 0x1
 *    Purpose = 1  ( Consent )
 ...
 *    AES Key =  0x75ab8bc72f3e2b201e0d0146dff8dfdcbc0c9581ba729cf39145ad459bea745a
...
func TestPulsePQ_KnownValues(t *testing.T) {
	...
	if !bytes.Equal(result.Keys[0].EncapsulatedKey, expectedKey1Key) {
		t.Fatalf("first key mismatch: got %x want %x", result.Keys[0].EncapsulatedKey, expectedKey1Key)
	}
	if !bytes.Equal(result.Keys[1].EncapsulatedKey, expectedKey2Key) {
		t.Fatalf("second key mismatch: got %x want %x", result.Keys[1].EncapsulatedKey, expectedKey2Key)
	}
...
func TestPulsePQ_SettersAndGetEncryptionResult(t *testing.T) {
	...
	e.encapsulatedKeys = []*PulsePQEncryptionKey{
		{KeyFingerPrint: finger1, EncapsulatedKey: enc1},
		{KeyFingerPrint: finger2, EncapsulatedKey: enc2},
	}
	e.ciphertext = cipher
...
func TestPulsePQ_Encrypt_Success_WithRecipients(t *testing.T) {
	...
	pubBuf1 := make([]byte, kyberKEM.PublicKeySize)
	pk1.Pack(pubBuf1)
	pkePub1 := new(kyberPKE.PublicKey)
	pkePub1.Unpack(pubBuf1)
	fp1 := getPubKeyFingerprint(pkePub1)

	pubBuf2 := make([]byte, kyberKEM.PublicKeySize)
	pk2.Pack(pubBuf2)
	pkePub2 := new(kyberPKE.PublicKey)
	pkePub2.Unpack(pubBuf2)
	fp2 := getPubKeyFingerprint(pkePub2)
...
func TestPulsePQ_GetEncryptionResult_SkipsNil(t *testing.T) {
	...
	if res.Keys[0] == nil || len(res.Keys[0].KeyFingerPrint) == 0 || len(res.Keys[0].EncapsulatedKey) == 0 {
...
func TestPulsePQ_Decrypt_TamperedEncapsulatedKey_Fails(t *testing.T) {
	...
	buf := make([]byte, kyberKEM.PublicKeySize)
	pk1.Pack(buf)
	pke1 := new(kyberPKE.PublicKey)
	pke1.Unpack(buf)
	fp1 := getPubKeyFingerprint(pke1)

	found := false
	for _, k := range res.Keys {
		if bytes.Equal(k.KeyFingerPrint[:], fp1[:]) {
			if len(k.EncapsulatedKey) > 0 {
				k.EncapsulatedKey[0] ^= 0xFF // flip a bit
			}
			found = true
			break
		}
	...
	if err := dec.Decrypt(); err == nil {
		t.Fatalf("expected decrypt failure with tampered encapsulated key")
	}
}
...
func TestPulsePQ_Encrypt_DeterministicWithSeed(t *testing.T) {
	...
	if !bytes.Equal(r1.Keys[0].EncapsulatedKey, r2.Keys[0].EncapsulatedKey) ||
		!bytes.Equal(r1.Keys[1].EncapsulatedKey, r2.Keys[1].EncapsulatedKey) {
		t.Fatalf("encapsulated keys should be identical with fixed seed and AESkey")
	}
	...
	if bytes.Equal(r1.Keys[0].EncapsulatedKey, r3.Keys[0].EncapsulatedKey) ||
		bytes.Equal(r1.Keys[1].EncapsulatedKey, r3.Keys[1].EncapsulatedKey) {
