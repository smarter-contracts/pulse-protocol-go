...
This is distinct from the "Key Encapsulation" method that uses Kyber768 to create recipient-specific keys
...
The encryption keys are derived from a BIP-32 HD wallet using the Pulse derivation path.
...
|-------|--------------------------------------------------|------------------------------|
| 2     | Consent notary block                             | encrypt-consent-notary-block |
| 3     | Consent data                                     | consent                      |
| 4     | Revoke notary block                              | encrypt-revoke-notary-block  |
| 5     | Revoke data                                      | revoke                       |
...
infoKeyString := SPRINTF("|pulse|kdf|v1|aead:channel:key|ecdh-secp256k1+hkdf-keccak256|rid=|ctx=%s|", HEX(hkdfContextHash))
aesKey := EXPAND(prk, infoKeyString, 32)   // Derive AES key for encrypting the data.
infoNonceString := SPRINTF("|pulse|kdf|v1|aead:channel:nonce|ecdh-secp256k1+hkdf-keccak256|rid=|ctx=%s|", HEX(hkdfContextHash))
...
```
myPublicKey := PUBKEY(myPrivateKey)
otherPublicKey := (encryptedData.Key1 == myPublicKey) ? encryptedData.Key2 : encryptedData.Key1

contextString := SPRINTF("|pulse|ctx|v1|chain=%d|contract=%s|consentNumber=%d", chainId, smartContractAddress, consentNumber)
contextHash := H(contextString)

keys = [ encryptedData.Key1, encryptedData.Key2 ]
SORT(keys)
transcriptString := SPRINTF("|pulse|group|v1|%s|%s|ecdh-secp256k1+hkdf-keccak256+aes-gcm-256|", keys[0], keys[1])
transcriptHash := H(transcriptString)
...
infoKeyString := SPRINTF("|pulse|kdf|v1|aead:channel:key|ecdh-secp256k1+hkdf-keccak256|rid=|ctx=%s|", HEX(hkdfContextHash))
aesKey := EXPAND(prk, infoKeyString, 32)
infoNonceString := SPRINTF("|pulse|kdf|v1|aead:channel:nonce|ecdh-secp256k1+hkdf-keccak256|rid=|ctx=%s|", HEX(hkdfContextHash))
aesNonce := EXPAND(prk, infoNonceString, 12)
...
| Purpose              | Purpose of the consent transaction                                         | 3 (consent)                                                      |
...
| prk                     | EXTRACT(sharedSecret,salt)                                    | b37289fa18c1c6b48da35bde046425f1fe31eb2ff1bc0c96ba133d6916f7aeab                                                                                                                                                                                                         |
| hkdfContextHash         | H(contextHash)                                                | 4cdac3f08f1d9b30e13c4bee9d3fbbaccb1717f4467778c0c0dfbe8b41f46862                                                                                                                                                                                                         |
...
| encryptionResult (CBOR) | CBOR.MARSHALL(encryptionResult)                               |  |
| encryptionResult (JSON) | JSON.MARSHALL(encryptionResult)                               | {"sealedData":"j4hSqxa7CVlrnYzpSnSCrHFdrNcRU3h4pIptdigoe6o0I6BTU0ZZM3Xu","key1":"A21sqsJIr5b2r6f5BPVQJToPPvP1qi/mg4qVshZpFGji","key2":"AxMTQeshVN3tEuOOC84D+QaAL7EGkOwbKycwOkqfuoi8"}                                                                                    |
