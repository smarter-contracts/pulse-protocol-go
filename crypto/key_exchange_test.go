...
func TestPulseECEncryption_RoundTrip_WithResult(t *testing.T) {
	...
	res, err := key_exchange.EncryptECDH(pt, addr, alicePriv, bobPub, purposes.PulsePurposeEncryptConsentStructure, 0x01, 0)
	if err != nil {
	...
	decrypted, err := key_exchange.DecryptEC(res, addr, bobPriv, purposes.PulsePurposeEncryptConsentStructure, 0x01, 0)
	if err != nil {
...
func TestPulseECEncryption_Encrypt_Errors(t *testing.T) {
	...
	// Missing private key
	_, err := key_exchange.EncryptECDH(pt, addr, nil, bobPub, purposes.PulsePurposeEncryptConsentStructure, 0x01, 0)
	if err == nil {
		t.Fatal("expected error with nil private key")
	}

	// Missing other public key
	_, err = key_exchange.EncryptECDH(pt, addr, alicePriv, nil, purposes.PulsePurposeEncryptConsentStructure, 0x01, 0)
	if err == nil {
...
func TestPulseECEncryption_Decrypt_Errors(t *testing.T) {
	...
	_, err = key_exchange.DecryptEC(res, addr, wrongPriv, purposes.PulsePurposeEncryptConsentStructure, 0x01, 0)
	...
	_, err = key_exchange.DecryptEC(&resTampered, addr, bobPriv, purposes.PulsePurposeEncryptConsentStructure, 0x01, 0)
	if err == nil {
