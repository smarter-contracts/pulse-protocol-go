...
func TestRevokeSignerWasConsentSigner_Valid(t *testing.T) {
	...
	// Create consent
	consentReq, err := EncryptSignConsentEC(wallet, []byte("consent payload"), otherParty, consent, bobPub, contractAddr, chainId)
	if err != nil {
		t.Fatalf("EncryptSignConsentEC() failed: %v", err)
	}
	...
	revokeReq, err := EncryptSignRevokeEC(wallet, []byte("revoke payload"), otherParty, consent, bobPub, contractAddr, chainId, consentCid.String())
	if err != nil {
...
func TestRevokeSignerWasConsentSigner_DifferentKey(t *testing.T) {
	 ...
	 */
	wallet := mustNewTestWallet(t)
	differentWallet := mustNewTestWallet(t) // same seed, but imagine a different key
	_, bobPub := mustNewOtherPartyKey(t)
	addr := helperContractAddress()
	contractAddr := *addr
	const otherParty, consent, chainId = uint32(2), uint32(62), uint32(1)

	consentReq, err := EncryptSignConsentEC(wallet, []byte("consent"), otherParty, consent, bobPub, contractAddr, chainId)
	if err != nil {
		t.Fatalf("EncryptSignConsentEC() failed: %v", err)
	}
	...
	revokeReq, err := EncryptSignRevokeEC(differentWallet, []byte("revoke"), otherParty+1, consent, bobPub, contractAddr, chainId, consentCid.String())
	if err != nil {
		t.Fatalf("EncryptSignRevokeEC() failed: %v", err)
	}
...
func TestRevokeSignerWasConsentSigner_MultipleConsentSigners(t *testing.T) {
	...
	if err = SignConsentRequest(wallet, consentReq, consentCBORForSign, otherParty, consent, contractAddr, chainId); err != nil {
		t.Fatalf("second SignConsentRequest() failed: %v", err)
	}
	...
	revokeReq, err := EncryptSignRevokeEC(wallet, []byte("revoke"), otherParty, consent, bobPub, contractAddr, chainId, consentCid.String())
	if err != nil {
		t.Fatalf("EncryptSignRevokeEC() failed: %v", err)
	}
...
func TestRevokeSignerWasConsentSigner_NilInputs(t *testing.T) {
	...
	consentReq, _ := EncryptSignConsentEC(wallet, []byte("consent"), otherParty, consent, bobPub, contractAddr, chainId)
	consentCBOR, _ := ipfs.MarshalConsentEC(&consentReq.EncryptedData)
	consentCid, _ := ipfs.GetCid(consentCBOR)
	revokeReq, _ := EncryptSignRevokeEC(wallet, []byte("revoke"), otherParty, consent, bobPub, contractAddr, chainId, consentCid.String())
...
func TestConsentSigners_AddressIsStable(t *testing.T) {
	...
	r1, err := EncryptSignConsentEC(wallet, []byte("data"), otherParty, consent, bobPub, contractAddr, chainId)
	if err != nil {
		t.Fatalf("first EncryptSignConsentEC() failed: %v", err)
	}
	r2, err := EncryptSignConsentEC(wallet, []byte("data"), otherParty, consent, bobPub, contractAddr, chainId)
	if err != nil {
