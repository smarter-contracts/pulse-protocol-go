...
type DecodedConsentBlock struct {
	Kind RecordKind
	V2EC *pptypes.PulseECEncryptionResult  // non-nil when Kind == RecordKindV2EC
	V2PQ *pptypes.PulsePQEncryptionResult  // non-nil when Kind == RecordKindV2PQ
...
// Exactly one of the pointer fields is non-nil; Kind identifies which.
// Both V2EC and V2PQ carry a PulseRevokePayload; for V2EC the EC fields
// (Key1, Key2) are populated; for V2PQ the PQ field (Keys) is populated.
type DecodedRevokeBlock struct {
	Kind RecordKind
	V2EC *pptypes.PulseRevokePayload  // non-nil when Kind == RecordKindV2EC
	V2PQ *pptypes.PulseRevokePayload  // non-nil when Kind == RecordKindV2PQ
